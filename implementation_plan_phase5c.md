...
> 4. **MCP 設定（Section C3）**：readwise-mcp、rss-reader-mcp、gmail-mcp、anybox-mcp 設定寫入 `.claude/settings.local.json`，視實際需求啟用
...
- `[NEW] canonical/workflows/readwise-sync.md`
- `[NEW] canonical/workflows/rss-sync.md`
...
- `[MODIFY] bin/agent`（+4 commands + help）：
  ```
  learning-note --url/--file/--readwise/--rss   文章 → 學習筆記
...
- **Anybox MCP**：非 PyPI 套件，需手動 clone + 設定 Python 路徑。且需 Anybox app 運行中
