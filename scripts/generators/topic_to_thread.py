...
from lib.config import TOPICS_DIR, THREADS_DIR, STYLE_DNA_DIR, RULES_DIR
...
- 📎 主文必帶素材出處可點 URL（在素材引用句下加「連結：https://...」）
- 🗣️ 保留 Dex raw 原話 ≥ 2 條（口頭禪、具體詞）
- 💎 主文 + 串文各至少 1 句金句可截圖
- 💬 串文末以提問或開放金句收尾引留言
- 🚫 不用 emoji（Threads style 禁忌）
...
def main():
    ...
    draft_dir = ensure_dir(THREADS_DIR / created_date)
    draft_path = draft_dir / f"{slug}.md"

    if draft_path.exists() and not args.force:
        response = input(f"[topic-to-thread] {draft_path.name} 已存在，覆蓋？(y/N) ").strip().lower()
        if response != "y":
...
"""
    ...
    try:
        ...
        if not topic_id:
            print(f"[topic-to-thread] 警告：topic_notion_map 查無 '{slug}'，Content 不會連到 Topic", file=sys.stderr)
        sync_draft_v2(
            title=topic_title,
            channel="threads",
