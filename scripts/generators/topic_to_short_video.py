...
from lib.config import TOPICS_DIR, RULES_DIR
...
def main():
    ...
    if not args.slug:
        ...
        for slug, title in topics:
            draft_exists = (TOPICS_DIR / slug / "short-video-script.md").exists()
            status = " [已有腳本]" if draft_exists else ""
            print(f"  {slug}{status}")
            print(f"    {title}")
    ...
    # 2. 檢查輸出
    draft_path = topic_dir / "short-video-script.md"
    if draft_path.exists() and not args.force:
        response = input(f"[topic-to-short-video] short-video-script.md 已存在，覆蓋？(y/N) ").strip().lower()
        if response != "y":
