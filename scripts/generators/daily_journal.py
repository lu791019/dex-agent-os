...
讀取 L1 工作日誌 → 用 claude --print 提煉 → 產出 L2 精煉日記。
...
SYSTEM_PROMPT = """\
你是 Dex 的個人 AI 代理人，負責將詳細的工作日誌（L1）提煉成簡潔的每日精煉日記（L2）。
...
def build_prompt(date_str: str, work_log_content: str, template_content: str) -> str:
    """組裝送給 LLM 的提示。"""
    return f"""\
請根據以下工作日誌，產出一份精煉日記。
...
def main():
    ...
    # 4. 呼叫 LLM 提煉
    prompt = build_prompt(date_str, work_log_content, template_content)
