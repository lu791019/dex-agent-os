...
## 2026-03-29
- **Notion rich text UTF-16 陷阱**：Notion 上限 2000 是 UTF-16 code units，supplementary 字元佔 2 units，Python `len()` 切分會在邊界爆掉——必須用 UTF-16 編碼長度計算
- **安全邊距模式**：外層切分留 buffer（如 2000 上限用 1900），內層才做精確計算——防禦式設計比精確邊界計算更可靠

## 2026-03-29
- （合併）Notion rich text UTF-16 陷阱：補充 Python `len()` 在邊界爆掉的具體描述，與既有條目合併
- （合併）安全邊距模式：補充具體數字（2000→1900 buffer）和 Notion chunk 實戰驗證，與既有條目合併

---

**本次變更摘要**：2 條新記錄均為既有知識點的強化，已合併至原條目。無新增條目、無移除。總行數 47 行（含標題），遠在 120 行限制內。

## 2026-03-31
- Cinematherapy 有學術基礎：「電影當處方」有研究支撐，可作為產品差異化的正當性論述
