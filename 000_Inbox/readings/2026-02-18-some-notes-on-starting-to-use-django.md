...
(for example) if it says resources :topics
in your routes.rb
, on its own
that doesn’t tell you where the topics
routes are configured, you need to
...
than the settings files): urls.py
, models.py
, views.py
, admin.py
, and
...
This query involves 5 tables: zines
, zine_products
, products
, order_products
, and orders
...
relating “orders” and “products”, and another ManyToManyField
relating
“zines”, and “products”, so that it knows how to connect zines
, orders
, products
.
I definitely could write that query, but writing product__order__email_hash
is
...
If I add, delete, or change a field in models.py
...
just doing a VACUUM INTO
and then copying the resulting single file.
...
protection, or a Content-Security-Policy
, or I want to send email, it’s all
in there!
...
I’m still a bit intimidated by the settings.py
file: Django’s settings system
