...
- [x] 設計決策討論完成（13 項決策，見 implementation_plan 的「設計決策」區塊）
...
- [ ] B2. 建立 `scripts/generators/learning_note.py` — 四種模式（--url / --file / --readwise [--all] / --rss），含 --type 和 --force
  - --url 和 --rss 使用 web_extract.py 的 fallback 鏈
...
- [ ] C3. 建立 `config/subscriptions.opml` — 初始 RSS 訂閱清單
- [ ] C4. 建立 `canonical/workflows/readwise-sync.md` + `rss-sync.md`
- [ ] C5. 更新 `.claude/settings.local.json` — MCP 設定（readwise / rss-reader / gmail / anybox）
- [ ] C6. 驗證：`./bin/agent readwise-sync --latest 3 --force`
- [ ] C7. 驗證：`./bin/agent rss-sync --feed "https://jvns.ca/atom.xml" --latest 3 --force`
- [ ] C8. 驗證跨模組：`./bin/agent learning-note --readwise --latest 1 --force`
- [ ] C9. 驗證跨模組：`./bin/agent learning-note --rss "https://jvns.ca/atom.xml" --latest 1 --force`

## 5c-1 收尾
- [ ] 更新 `bin/agent` — 新增 3 指令（learning-note / readwise-sync / rss-sync）+ help
- [ ] 確保測試資料存在：`learning-note --file` + `rss-sync --feed` 產出（不依賴 Readwise）
- [ ] Commit + merge 到 main
